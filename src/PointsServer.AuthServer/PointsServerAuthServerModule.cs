...
using PointsServer.MultiTenancy;
...
public class PointsServerAuthServerModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        ...
        PreConfigure<OpenIddictBuilder>(builder =>
        {
            ...
            {
                options.AddAudiences("PointsServer");
    ...
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        {
            options.Grants.Add("signature", context.Services.GetRequiredService<SignatureGrantHandler>());
        });

        Configure<AbpLocalizationOptions>(options =>
        {
            options.Resources
                .Get<PointsServerResource>()
                .AddBaseTypes(
        ...
        if (hostingEnvironment.IsDevelopment())
        {
            ...
            {
                options.FileSets.ReplaceEmbeddedByPhysical<PointsServerDomainSharedModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}PointsServer.Domain.Shared"));
                options.FileSets.ReplaceEmbeddedByPhysical<PointsServerDomainModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}PointsServer.Domain"));
            });
        }
        ...
        {
            options.KeyPrefix = "PointsServer:Auth:";
        });

        var dataProtectionBuilder = context.Services.AddDataProtection().SetApplicationName("PointsServer");
        if (!hostingEnvironment.IsDevelopment())
        {
            var redis = ConnectionMultiplexer.Connect(configuration["Redis:Configuration"]);
            dataProtectionBuilder.PersistKeysToStackExchangeRedis(redis, "PointsServer-Protection-Keys");
        }
